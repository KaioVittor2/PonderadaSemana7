...
```
idade <= imputPrompt # idade colocada no prompt

se idade < 16 for verdade então:
    printar "Não pode votar"

senão, se idade < 18 for verdade então:
    printar "voto facultativo"

senão:
    printar "voto obrigatório"
```
______
...
Classe Retangulo extendido de FormaGeometrica:
    ...
    Método Construtor(Cor, largura e Altura):
    Super do atributo cor da classe pai FormaGeometrica
    Define os valores dos demais atributos largura e Altura com os valores passados como parâmetro.

    Método CalcularArea():
    Utiliza do parâmetro largura multiplicado pelo parâmetro Altura
...
Classe Circulo extendido de FormaGeometrica:
    ...
    Método CalcularArea():
    Utiliza do parâmetro raio elevado ao quadrado multiplicado por pi
```
...
Classe Desempenho:
    ...
    Método Construtor(velocidadeInicial, aceleracao, tempo, distanciaTotal, tempoMaximo):
        Define os valores dos atributos velocidadeInicial, aceleracao, tempo, distanciaTotal e tempoMaximo com os valores passados como parâmetro.

    para (velocidade = 0, distanciaPercorrida = 0; velocidadeMaxima > velocidade; tempo++) faça:
        velocidade <= velocidadeInicial + aceleracao X tempo

        se distanciaPercorrida < distanciaTotal e também tempoMaximo > tempo então:
        distanciaPercorrida <= velocidade X tempo

        senão: parar

    se distanciaPercorrida < distanciaTotal e também tempoMaximo > tempo então:
    distanciaPercorrida <= velocidadeMaxima X tempo

    senão: printar o valor do tempo
```
______
        ...
        Para i <= 0 até linhasA - 1 faça:
            Para j <= 0 até colunasB - 1 faça:
                ...
                Para k de 0 até colunasA - 1 faça:
                    SomaDaPosicao <= SomaDaPosicao + matrizA[i][k] X matrizB[k][j]
                matrizResultado[i][j] <= SomaDaPosicao
